...
class ChessBoard:
    ...
    def __init__(self, path="", nx=9, ny=6, generation=False):
        """Class Constructor

        Instantiates class constants
        """
        self.path = path
        self.nx, self.ny = nx, ny
        self.n = (self.nx, self.ny)
        if generation:
            self.initialize_for_data_generation()

    def initialize_for_data_generation(self):
        """Initializes chessboard constants"""
        ...
        self.has_corners, self.corners = cv2.findChessboardCorners(
            temp_gray, self.n, None)
    ...
    def image_with_corners(self):
        ...
        temp_image = imread(self.path)
        if self.has_corners:
            cv2.drawChessboardCorners(temp_image, self.n, self.corners,
                                      self.has_corners)
        return temp_image
    ...
    def undistorted_image(self):
        ...
        if self.can_undistort:
            temp_image = imread(self.path)
            temp_image = cv2.undistort(temp_image, self.matrix, self.distortion,
                                       None, self.matrix)
        return temp_image

    def undistorted_image_with_data(self, image, data):
        undistorted_image = cv2.undistort(temp_image, data.camera_matrix,
                                          data.distortion_coefficient, None,
                                          data.camera_matrix)
        return undistorted_image

    def load_undistort_params(self, camera_matrix, distortion):
        """Use this method if you already have distortion parameters"""
